...
export default {
  ...
  methods: {
    ...
    sector(cx, cy, r, startAngle, ctx, percent) {
      ...
      linGrad.addColorStop(0, this.ID.startColor); //"#2038DE"
      linGrad.addColorStop(1, this.ID.endColor); //"#0DD3FF"
      ctx.strokeStyle = linGrad;
      ctx.lineCap = "round";
      if(this.ID.target < 0){
        if((2-(this.ID.percent / this.ID.target) < 0 )){
          ctx.arc(cx, cy, r, -Math.PI * 1 / 2, -Math.PI * 1 / 2);
        }else{
          ctx.arc(cx, cy, r, -Math.PI * 1 / 2,
           -Math.PI * 1 / 2 + 2 * Math.PI * ((2-(this.ID.percent / this.ID.target )) >= 1 ? 1:2-(this.ID.percent / this.ID.target ))
         )
        }
        ...
        if( this.ID.percent / this.ID.target < 0 ){
          ...
          ctx.arc(cx, cy, r, -Math.PI * 1 / 2, -Math.PI * 1 / 2 + this.process / this.ID.target * 2 * Math.PI);
        }
      } 
      ctx.stroke();
    },
    ...
    loading(circleX, circleY, radius, startAngle, ctx, percent) {
      ...
      if(!Number(this.ID.target)){
        ...
        circleY * 4 / 5 + 20
         ...
         ctx.fillText( this.ID.target == 0 ? "--" : (Math.round((2-(this.ID.percent / this.ID.target).toFixed(2)) * 100)
              ) + "%",
        circleX,
        circleY * 4 / 5 +20
        );
