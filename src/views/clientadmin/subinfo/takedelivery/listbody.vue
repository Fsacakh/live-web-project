<template>
  <div class="row">
    <div class="col-md-12">
      <div class="row">
        <div class="col-md-12">
          <b-button size="sm" variant="success" @click="insert" v-b-modal.insert4>新增</b-button>
          <b-button size="sm" variant="info" @click="update" v-b-modal.updata4>编辑</b-button>
        ...
        <table class="table table-striped table-bordered">
          ...
          <tbody class="text-center">
            <tr v-for="(item,index) in datalist" class="text-center">
              ...
              <td class="text-center">{{item.receiverMobilePhone}}</td>
  ...
  export default {
    ...
    methods: {
      ...
      querylist() {
        this.datalist = []
        this.datalistobj = {}
        api.clientadmin.clientreceiver.queryclientreceiver({
          ...
          if (msg.data.code == 'success') {
            ...
            this.$store.dispatch("clientmaininfo/amendclientreceiver", false);
          }
        })
      },
      removeTree(item) {
        MessageBox.confirm("是否删除该数据", "提示", {
          ...
          .then(() => {
            api.clientadmin.clientreceiver.updataclientreceiver({
              id: item.id,
      ...
      checked(item) {
        this.$store.dispatch("clientmaininfo/amendclientreceiver", item.receiverCode);
      }
    },
    mounted() {
      //获取客户编码
    ...
    computed: {
      ...mapState('clientmaininfo', [
        'queryclientreceiverdata',
      ]),
    },
    watch: {
      queryclientreceiverdata: function() {
        if (this.queryclientreceiverdata) {
          this.datalist = []
          this.datalistobj = {}
          api.clientadmin.clientreceiver.queryclientreceiver({
            receiverCode: this.queryclientreceiverdata,
            pageStart: 1,
            pageNums: this.pageNums,
          }, (msg) => {
            if (msg.data.code == 'success') {
