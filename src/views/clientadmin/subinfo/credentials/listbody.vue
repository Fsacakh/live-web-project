<template>
  <div class="row">
    <div class="col-md-12">
      <div class="row">
        <div class="col-md-12">
          <b-button size="sm" variant="success" @click="insert" v-b-modal.insert2>新增</b-button>
          <b-button size="sm" variant="info" @click="update" v-b-modal.updata2>编辑</b-button>
        ...
        <table class="table table-striped table-bordered">
          ...
          <tbody class="text-center">
            <tr v-for="(item,index) in datalist" class="text-center">
              ...
              <td class="text-center">{{item.certificateNumber}}</td>
              <td class="text-center">{{item.certificateTypeName}}</td>
  ...
  export default {
    ...
    methods: {
      ...
      querylist() {
        this.datalist = []
        this.datalistobj = {}
        api.clientadmin.clientidtype.queryclientidtype({
          ...
          if (msg.data.code == 'success') {
            ...
            this.$store.dispatch("clientmaininfo/queryidtype", false)
            this.$store.dispatch("clientmaininfo/amendidtype", false);
          }
        })
      },
      removeTree(item) {
      ...
      checked(item) {
        this.$store.dispatch("clientmaininfo/amendidtype", item.certificateCode);
      },
    },
    mounted() {
      //获取客户编码
    ...
    watch: {
      queryidtypedata: function() {
        if (this.queryidtypedata) {
          this.datalist = []
          this.datalistobj = {}
          api.clientadmin.clientidtype.queryclientidtype({
            certificateCode: this.queryidtypedata,
            pageStart: 1,
            pageNums: this.pageNums,
          }, (msg) => {
            if (msg.data.code == 'success') {
