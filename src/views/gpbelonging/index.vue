<template>
    <div class="animated fadeIn">
        ...
        <b-modal id="setbelonging" ref="setbelonging" :title="'设置GP归属'" ok-title="确定" cancel-title="取消" hide-footer="">
            <div class="col-md-10">
...
export default {
  ...
  data() {
    return {
      ...
      optfor: "",
      query: {
        gpDetailTypeCode: config.gpType[0].value,
  ...
  computed: {
    ...mapState("financing", ["gpDetail", "gpbelongingCode", "detailSelect"])
  },
  created() {
    this.getgptype();
  ...
  methods: {
    ...
    showModal: function(type) {
      ...
      if (type === "edit") {
        ...
        let serviceCode =
          $this.lei == "gp"
            ? config.financing.gpbelonging
            : config.financing.snpbelonging;
    ...
    queryGPdetailInfo: function() {
      const $this = this;
      let url;
      url = api["GPbelonging"].queryDetail;
      url($this.query, res => {
        if (res.data.code == "success") {
    ...
    getGPdetail: function() {
      ...
      let refCode =
        $this.lei == "gp"
          ? config.financing.getGPdetail
          : config.financing.getSNPdetail;
      $this.$store.dispatch("financing/getType", {
        poros: { refCode: refCode },
        callBack: function(msg) {
          if (msg.data.code == "success") {
