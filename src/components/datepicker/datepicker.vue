<template>
  <div class="datepicker">
    <input class="form-control datepicker-input" :class="{'with-reset-button': clearButton, 'is-invalid': state}" type="text" :placeholder="placeholder"
        :style="{width:width}"
        :disabled="disabled"
        :aria-invalid="true"
        @click="inputClick"
    ...
    <button v-if="clearButton && localValue" type="button" class="close" @click="localValue = ''">
      <span>&times;</span>
    </button>
          ...
          <div class="datepicker-monthRange">
            <template v-for="(m, index) in text.months">
              <span   :class="{'datepicker-dateRange-item-active':
                  (text.months[parse(localValue).getMonth()]  === m) &&
                  currDate.getFullYear() === parse(localValue).getFullYear()}"
        ...
        <div class="datepicker-body">
          ...
          <div class="datepicker-monthRange decadeRange">
            <template v-for="decade in decadeRange">
              <span :class="{'datepicker-dateRange-item-active':
                  parse(localValue).getFullYear() === decade.text}"
...
export default {
  ...
  methods: {
    ...
    inputClick () {
      this.currDate = this.parse(this.localValue) || this.parse(new Date())
    ...
    daySelect (date, cls) {
      if (cls === 'datepicker-item-disable') {
        return false
      } else {
        this.currDate = date
        this.localValue = this.stringify(this.currDate)
        this.displayDayView = false
      }
    },
    switchMonthView () {
    ...
    getDateRange () {
      ...
      for (let i = 1; i <= dayCount; i++) {
        ...
        if (i === time.day) {
          if (this.localValue) {
            const valueDate = this.parse(this.localValue)
            if (valueDate) {
